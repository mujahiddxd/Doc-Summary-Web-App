...
class Summarizer:
    ...
    def abstractive_summarize(self, text, num_sentences, prompt=''):
        # Estimate summary length based on number of sentences (assume ~20 words per sentence)
        max_length = num_sentences * 20
        min_length = max(30, num_sentences * 10)  # Ensure a minimum reasonable summary
        ...
        result = self.summarization_pipeline(text, max_length=max_length, min_length=min_length, do_sample=False)
        doc, sents = self._clean_text(text)
        word_weights = self._get_word_weights(doc)
        sentence_weights = self._get_sentence_weights(doc, word_weights)
        
        # Return the summarized text
        return word_weights, sentence_weights, sents,result[0]['summary_text']